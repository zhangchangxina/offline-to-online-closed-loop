...
class ClosedLoopSACAgent(SACAgent):
    ...
    def policy_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        # Compute lam_align with schedule: fixed | linear | lagrangian (only used in 'align' variant)
        ...
        if self.config.get("log_actor_grad_terms", False):
            ...
            def align_term_only_fn(p):
                ...
                lam_local = jnp.maximum(0.0, (
                    self.forward_align_lagrange(grad_params=p) if self.config.get("lambda_schedule", "fixed") == "lagrangian" else lam_align
                ))
    ...
    def align_lagrange_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        if (not self.config.get("closed_loop_enabled", False)) or (lambda_schedule != "lagrangian"):
    ...
    def loss_fns(self, batch):
        ...
        # Only expose align_lagrange loss when module exists (Lagrangian schedule)
        if self.config.get("lambda_schedule", "fixed") == "lagrangian":
    ...
    def create(
        ...
        **kwargs,
        ...
        # Only create align_lagrange module if lambda_schedule is lagrangian
        need_align_module = kwargs.get("lambda_schedule", "fixed") == "lagrangian"
        ...
        closed_loop_config = {
            ...
            # Current build includes align_lagrange module only if lambda_schedule=='lagrangian'
