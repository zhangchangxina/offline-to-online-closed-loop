...
echo "[GPU ${GPU_ID}] CQL (REDQ10, UTD=4) pretrain+online for ${ENV_ID}"
python3 finetune.py \
  --agent cql \
  ...
  --exp_name cql_ensemble_highutd \
  --save_dir ${SAVE_ROOT} \
  2>&1 | tee -a ${SAVE_ROOT}/cql_${ENV_ID}_seed${SEED}.log

EXP_DESC="cql_ensemble_highutd_${ENV_ID}_cql_seed${SEED}"
...
python3 finetune.py \
  ...
  --exp_name cql_ensemble_highutd_append \
  --save_dir ${SAVE_ROOT} \
  2>&1 | tee -a ${SAVE_ROOT}/cql_${ENV_ID}_seed${SEED}.log


echo "[GPU ${GPU_ID}] WSRL (SAC) from CQL-300K for ${ENV_ID}"
python3 finetune.py \
  --agent sac \
...
python3 finetune.py \
  ...
  echo "[GPU ${GPU_ID}] WSRL (SAC-BC) from CQL-300K for ${ENV_ID}"
python3 finetune.py \
  --agent sac_bc \
  --config experiments/configs/train_config.py:locomotion_wsrl \
  --env ${ENV_ID} \
